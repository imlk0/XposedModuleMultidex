...
/**
 * XMMultiDex patches {@link Context#getClassLoader() the application context class
 ...
 */
public final class XMMultiDex {

    static final String TAG = "XMMultiDex";

    private static final String OLD_SECONDARY_FOLDER_NAME = "xm_secondary-dexes";

    private static final String CODE_CACHE_NAME = "xm_code_cache";

    private static final String CODE_CACHE_SECONDARY_FOLDER_NAME = "xm_secondary-dexes";
     ...
     *
     * @param moduleClassLoader the classloader which load the module
     * @param MODULE_PATH module abslute path.
     * @param applicationInfo injected application's applicationInfo.
     * @throws RuntimeException if an error occurred preventing the classloader
     *                          extension.
     */
    public static void install(final ClassLoader moduleClassLoader, final String MODULE_PATH, final ApplicationInfo applicationInfo) {
        Log.i(TAG, "install");
        if (IS_VM_MULTIDEX_CAPABLE) {
            Log.i(TAG, "VM has multidex support, XMMultiDex support library is disabled.");
            return;
        }
        ...
        try {
            ...
            synchronized (installedApk) {
                ...
                if (installedApk.contains(MODULE_PATH)) {
                    return;
                }
                installedApk.add(MODULE_PATH);

                if (Build.VERSION.SDK_INT > MAX_SUPPORTED_SDK_VERSION) {
                    Log.w(TAG, "XMMultiDex is not guaranteed to work in SDK version "
                ...
                try {
                    ...
                    clearDexDir(applicationInfo.dataDir);
                } catch (Throwable t) {
                    Log.w(TAG, "Something went wrong when trying to clear old XMMultiDex extraction, "
                            + "continuing without cleaning.", t);
                }

                File dexDir = getDexDir(applicationInfo.dataDir, applicationInfo);
                List<File> files = MultiDexExtractor.load(MODULE_PATH, dexDir);
    ...
    private static void clearDexDir(String path) throws Exception {
        File cacheDir = new File(path,CODE_CACHE_NAME);
        File dexDir = new File(cacheDir, OLD_SECONDARY_FOLDER_NAME);
        if (dexDir.isDirectory()) {
            ...
            if (!cacheDir.delete()) {
                Log.w(TAG, "Failed to delete cache dex dir " + dexDir.getPath());
            } else {
                Log.i(TAG, "Deleted old cache dex dir " + dexDir.getPath());
            }
        }
    }

    private static File getDexDir(String path, ApplicationInfo applicationInfo)
            throws IOException {
        ...
        try {
             ...
             */
            cache = new File(path, CODE_CACHE_NAME);
            mkdirChecked(cache);
        }
